...
class EDGARDownloader:
    def __init__(self, base_dir = BASE_DATA_DIR, tart_dir = Path('D:/EDGAR_data'), rate_limit = 0.15):
    ...
    def download_filing(self, filling: Dict) -> Dict:
        ...
        has_xbrl_format = 0
        targets = ['10-K', '10-Q', '10-K/A', '10-Q/A']
        pattern = re.compile(r'10[-]?K(?:/A)?|10[-]?Q(?:/A)?', re.IGNORECASE)


        if any(t in key.upper() or key.upper() in t for key in urls.keys() for t in targets) or any(pattern.search(key) for key in urls.keys()):
            try:
