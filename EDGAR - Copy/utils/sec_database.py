...
class SECDatabase:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''
                CREATE TABLE IF NOT EXISTS filings (
                    ...
                    has_xbrl_format INTEGER DEFAULT 0,  -- 0=False, 1=True,
                    path TEXT,
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
                    ...
                    (filing_id, cik, accession_number, form_type, company_name, ticker,
                     ...
                     file_path, file_count, total_size, has_xbrl_format, path, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)
                ''', (
                    ...
                    filing_data.get('has_xbrl_format', 0),
                    filing_data.get('path', ''),
    ...
    def print_filings_by_cik(self, cik: str):
        """Print all filing records for a given CIK"""
        query = 'SELECT * FROM filings WHERE cik = ? ORDER BY filing_date DESC'
        
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row  # enables dict-style access
            cursor = conn.execute(query, (cik,))
            ...
            print(f"Found {len(rows)} filings for CIK {cik}:\n")
