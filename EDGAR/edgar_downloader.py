...
class EDGARDownloader:
    def __init__(self, base_dir = BASE_DATA_DIR, rate_limit = 0.17):
    ...
    def create_folder_path(self, cik: str, year: str, form_type: str, company_name:str) -> Path:
        ...
        new_subpath = f"{year}_{form_type}"
        path = [self.base_dir, cik, company_name, new_subpath]
        target_dir = Path(*path)
        target_dir.mkdir(parents=True, exist_ok=True)

        path2 = [Path("D:/EDGAR_data"), cik, company_name, new_subpath]
        target_dir2 = Path(*path2)
        target_dir2.mkdir(parents=True, exist_ok=True)
        return target_dir, target_dir2

    def create_filename(self, fiscal_info: Dict, report_date, filing_date) -> Dict:
        ...
        if form in ['10-Q', '10-Q/A']:
            ...
            if fiscal_info.get('fiscal_year') and fiscal_info.get('fiscal_period'):
                clean_date = f"{fiscal_info['fiscal_year']}_{fiscal_info['fiscal_period']}"
        ...
        return f"{clean_form}_{clean_date}"
    ...
    def get_tikcer_by_cik(self, cik:str):
        cik = cik.strip()
        if self.company_tickers[cik]:
            return self.company_tickers[cik]
        return 'UNKNOWN'

    def get_company_recent_filings(self, cik: str, accession_number: any) -> Dict:
        ...
        try:
            ...
            for i, form in enumerate(filings['form']):
                ...
                if ac.replace(" ", "") == accession_number and form_type in form_types:
    ...
    def download_filing(self, filling: Dict) -> Dict:
        ...
        file_name = self.create_filename(fiscal_info, fiscal_info['period_end_year'], fiscal_info['date_filed_year'])
        ...
        if any(k in urls for k in ['10-K', '10-Q', '10-K/A', '10-Q/A']):
            try:
                for key, url in urls.items():
                    if key == form_type:
                        html_path = target_dir/f"{file_name}.html"
                    else:
                        html_path = target_dir/f"{file_name}_{key.replace("/", "_")}.html"
                    ...
                    updated_html = re.sub(
                        r'<img[^>]*src=["\']([^"\']*\.jpg)["\']',
                        lambda m: m.group(0).replace(m.group(1), f"https://www.sec.gov/Archives/edgar/data/{cik}/{accession_number.replace('-', '')}/{Path(m.group(1)).name}"),
                        html_content,
                        flags=re.IGNORECASE
                    )
            ...
            try:
                txt_path = target_dir/f"{file_name}_full_submission.txt"
        ...
        if int(fiscal_year) >= 2009:
            try:
                ...
                if xlsx_resp.status_code == 200:
                    ...
                    self.logger.info(f"[{ticker if ticker != 'UNKNOWN' else (company_name if company_name != 'UNKNOWN' else cik)} Saved financial report xlsx to {txt_path}]")
        ...
        if results['downloaded_files']:
            filing_data = {
                ...
                'filing_date': filling['date_filed'],
                ...
                'total_size': sum(Path(f).stat().st_size for f in target_dir.iterdir()),
                'download_status': 'completed'
            }
            self.database.add_filing(filing_data)
        
        
        return results, target_dir, target_dir2
        

if __name__ == "__main__":
    downloader = EDGARDownloader(base_dir='test-data')
    # cik = downloader.get_cik_by_ticker('ACU')
    filings = downloader.get_company_recent_filings('0000001961', '10-K')
    downloader.download_filing(filings[-2])
